...
class TransformerPrediction(nn.Module):
    def __init__(self, config, feature_length=39, label_length=31, num_layers=2, num_heads=8, hidden_size=768):
...
class GlocalTextPathCMTPreTraining(BertPreTrainedModel):
    ...
    def get_room_type_for_view(self, scan, cand_vpids):
        " 得到基于视觉的房间分类 "
        # print('room_labels:', self.room_labels['EDJbREhghzL']['a1dd7b2789d64b5a8e9adcd864ad9897'])
        all_list = torch.LongTensor([])
    ...
    def forward(self, batch, task, compute_loss=True):
        ...
        if task.startswith('mlm'):
            ...
            return self.forward_rc(
                ...
                batch['traj_vpids'], batch['scan'], compute_loss
            )
    ...
    def forward_rc(
        ...
        region_soft_labels = self.region_classifier(region_fts)  # .reshape(region_fts.shape[0], -1)
